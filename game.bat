...
IF "%1"=="build" (
    IF "%LOG_OPTION%"=="terminal" (
        ...
        javac -d bin -cp lib/jars/lwjgl.jar;lib/jars/lwjgl_util.jar;lib/jars/slick-util.jar src/engineTester/MainGameLoop.java src/renderEngine/*.java
        echo Compilação concluída.
    ) ELSE (
        echo Compilando arquivos... >> %LOG_FILE%
        javac -d bin -cp lib/jars/lwjgl.jar;lib/jars/lwjgl_util.jar;lib/jars/slick-util.jar src/engineTester/MainGameLoop.java src/renderEngine/*.java >> %LOG_FILE% 2>&1
